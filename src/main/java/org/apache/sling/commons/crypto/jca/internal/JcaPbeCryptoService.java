 ...
 * It relies on  Password-Based key derivation function (PBKDF) for key derivation and a symmetric cipher for encryption and decryption.
...
public final class JcaPbeCryptoService implements CryptoService {
    ...
    private final Logger logger = LoggerFactory.getLogger(JcaPbeCryptoService.class);
    ...
    private SecretKey createKey(final JcaPbeCryptoServiceConfiguration configuration, byte[] salt) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, InvalidParameterSpecException, InvalidKeySpecException {
        ...
        SecretKeyFactory secretKeyFactory = SecretKeyFactory.getInstance(configuration.secretKeyFactory(), provider);
        // wrap as key for the proper cipher algorithm (e.g., AES) instead of the PBE algorithm (e.g., PBKDF2WithHmacSHA512)
        return new SecretKeySpec(secretKeyFactory.generateSecret(keySpec).getEncoded(), extractAlgorithmName(configuration.cipherAlgorithm()));
    }
    ...
    /**
     * @param iv if null, the cipher will be initialized for encryption, otherwise for decryption
    ...
    public @NotNull String encrypt(@NotNull final String message) {
        ...
        try {
            ...
            byte[] paramsName = cipherEncrypt.getParameters().getAlgorithm().getBytes(StandardCharsets.UTF_8);
            // Combine parameters + ciphertext into single array
            byte[] result = new byte[Integer.BYTES + paramsName.length + Integer.BYTES + paramLength + cipherTextBytes.length];
            int offset = 0;
    ...
    public @NotNull String decrypt(@NotNull final String cipherText) {
        ...
        try {
            ...
            // Step 1: Extract length of params, params and ciphertext (remaining)
            int offset = 0;
            ...
            offset += paramsLength;
            byte[] cipherData = new byte[encryptedData.length - offset];
            System.arraycopy(encryptedData, offset, cipherData, 0, cipherData.length);
            Cipher cipher = createCipher(paramsNameStr, params);
